...
from slv_orders_functions import ORDER_DQ_RULES, prepare_orders
...
def orders_validated():
    source_df = spark.readStream.table(f"{CATALOG}.{BRONZE_SCHEMA}.brz_orders")  # noqa: F821

    return prepare_orders(source_df).select(
        "order_id",
        "customer_id",
        "product_id",
        "quantity",
...
def orders_silver():
    return (
        spark.readStream.table("orders_validated")  # noqa: F821
        .filter(F.col("is_quarantined") == F.lit(False))
        .drop("is_quarantined", "quarantine_reason")
        .withWatermark("ingest_datetime", "1 day")
        .dropDuplicatesWithinWatermark(["order_id", "product_id"])
        .select(
            "order_id",
            "customer_id",
            "product_id",
...
def quarantine_orders():
    return spark.readStream.table("orders_validated").filter(F.col("is_quarantined") == F.lit(True)).select(  # noqa: F821
        "order_id",
        "customer_id",
        "product_id",
        "quantity",
